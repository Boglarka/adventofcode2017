...
var input = `mmqyju (156) -> rjzvwv, noybkx
...
func task1() string {
  word, words := "", map[string][]string{}
  rows := strings.Split(input, "\n")
  for i := 0; i < len(rows); i++ {
    if strings.Contains(rows[i], "->") {
      row := strings.Split(rows[i], " -> ")
      part1 := strings.Split(row[0], " ")
      part2 := strings.Split(row[1], ", ")
      words[part1[0]] = part2
    }
  }
  ...
  return getParent(words, word)
}

func task2() int {
  word, words := "", map[ww][]string{}
  rows := strings.Split(input, "\n")
  for i := 0; i < len(rows); i++ {
    if strings.Contains(rows[i], "->") {
      row := strings.Split(rows[i], " -> ")
      part1 := strings.Split(row[0], " ")
      part2 := strings.Split(row[1], ", ")
      //num := strings.Split(part[2], "(")
      //num := int(part2)
      num := 1
      fmt.Println(part1)
      words[part1[0]: num] = part2
      return 0
    }
  }
  return 0
...
func getParent(words map[string][]string, word string) string {
  for key, value := range words {
    for _, val := range value {
      if word == val {
        return getParent(words, key)
      }
    }
  }
  return word
