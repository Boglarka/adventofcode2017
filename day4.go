...
func task2() int {
  ...
  for i := 0; i < len(rows); i++ {
    ...
    for j := 0; j < len(words)-1; j++ {
      for k := j+1; k < len(words); k++ {
        if len(words[j]) == len(words[k]) && (words[j] == words[k] || isAnagram(words[j], words[k])) {
          ok = false
          break
        }
      }
...
func isAnagram(w1, w2 string) bool {
  w1a := strings.Split(w1, "")
  w2a := strings.Split(w2, "")
  sort.Sort(Alphabetic(w1a))
  sort.Sort(Alphabetic(w2a))
  for i, char := range w1a {
    if char != w2a[i] {
      return false
    }
  }
